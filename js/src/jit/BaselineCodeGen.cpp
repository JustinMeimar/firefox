...
static void emitAsmBytes(std::ofstream& out, const uint8_t* data, size_t len) {
...
static void emitAsmGlobal(std::ofstream& out, const char* name) {
  out << ".global " << name << "\n";
}

static void emitAsmLong(std::ofstream& out, const char* sym, uint32_t val) {
  emitAsmGlobal(out, sym);
  out << sym << ":    .long 0x" << std::hex << std::setfill('0') << std::setw(8)
      << val << std::dec << "\n";
}

static void emitAsmVec(std::ofstream& out, const char* name,
                       const uint8_t* data, size_t len) {
  char startSym[128], endSym[128];
  snprintf(startSym, sizeof(startSym), "bl_aot_%s_start", name);
  snprintf(endSym, sizeof(endSym), "bl_aot_%s_end", name);
  out << ".balign 4\n";
  emitAsmGlobal(out, startSym);
  emitAsmGlobal(out, endSym);
  out << startSym << ":\n";
  if (len > 0) {
...
bool BaselineInterpreterGenerator::serializeAOTManifest(JitCode* code) {
  ...
  // Code section
  out << ".balign 16\n";
  emitAsmGlobal(out, "bl_aot_code_start");
  emitAsmGlobal(out, "bl_aot_code_end");
  out << "bl_aot_code_start:\n";
  emitAsmBytes(out, codeStart, codeSize);
  out << "bl_aot_code_end:\n\n";

  // Manifest scalars via X-macro
  out << ".balign 4\n";
#define EMIT_MANIFEST_FIELD(name) \
  emitAsmLong(out, "bl_aot_" #name, m[uint32_t(F::name)]);
  BASELINE_MANIFEST_FIELDS(EMIT_MANIFEST_FIELD)
#undef EMIT_MANIFEST_FIELD
  out << "\n";

  // Vector payloads
  auto emitVecData = [&](const char* name, const auto& vec) {
  ...
  emitVecData("DebugInstrumentationOffsets",
              handler.debugInstrumentationOffsets());
  emitVecData("DebugTrapOffsets", aot_->accumulator().debugTraps);
  emitVecData("CodeCoverageOffsets", handler.codeCoverageOffsets());
  emitVecData("ICReturnOffsets", handler.icReturnOffsets());
  emitVecData("RuntimePatches", aot_->accumulator().runtimePatches);
