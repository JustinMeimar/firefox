...
// All emitAsm* helpers use the "bl_aot_" prefix internally — callers pass
// bare names (e.g. "InterpretOp", "RuntimePatches").
...
bool BaselineInterpreterGenerator::serializeAOTManifest(JitCode* code) {
  ...
  // Code blob — emitAsmVec gives us start/end labels + .balign.
  out << ".balign 16\n";
  out << ".global bl_aot_code_start\n";
  out << ".global bl_aot_code_end\n";
  out << "bl_aot_code_start:\n";
  emitAsmBytes(out, codeStart, codeSize);
  out << "bl_aot_code_end:\n\n";

  // Manifest scalars — X-macro drives emission.
  out << ".balign 4\n";
