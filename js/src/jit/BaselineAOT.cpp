...
uintptr_t PatchRegistry::Resolve(PatchHandlerID id, CompileRuntime* cx, const PatchContext& ctx, uintptr_t payload) {
    MOZ_ASSERT(s_PatchHandlers[id]);
    return s_PatchHandlers[id](cx, ctx, payload);
}

void InitBaselinePatches() {
    PatchRegistry::Register(WellKnownSymbolPatch::ID, WellKnownSymbolPatch::Resolve);
