...
void MacroAssembler::checkAllocatorState(Register temp, gc::AllocKind allocKind,
  ...
  // TODO(aot): confirm skipping this guard during AOT fill is sound. The
  // hasRealmWithAllocMetadataBuilder check is a compile-time property,
  // whereas AOT ICs bind their realm and zone at runtime.
...
#ifdef DEBUG
#  ifdef ENABLE_JS_AOT
  // The class-pointer comparison needs an indirection slot for the class.
  // Skip the assertion when no slot is registered.
  if (isAOT() && !aotTable().findSlot(uintptr_t(clasp))) {
    return;
  }
#  endif
  Label done;
